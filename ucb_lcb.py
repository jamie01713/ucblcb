        ...
        for t in range(1,T+1):
            ...
            for i in range(N):
                if i in active:
                    ...
                    n_pull[i,state[i]]+=1
                    LCB[i,state[i]]=mu[i,state[i]]/n_pull[i,state[i]]-math.sqrt(math.log(n_pull[i,int(state[i])]+2)/(n_pull[i,int(state[i])]+2))-nu0[i,int(state[i])]
                    UCB[i,state[i]]=mu[i,int(state[i])]/n_pull[i,int(state[i])]+1.714*math.sqrt((math.log(math.log(n_pull[i,int(state[i])]+2))+2*math.log(T*10))/(n_pull[i,int(state[i])]+1))-nu0[i,int(state[i])]
