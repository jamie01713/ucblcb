...
if __name__ == '__main__':
    ...
    use_algos = ['optimal', 'ucw_value', 'ucw_qp', 'ucw_extreme', 'wiql', 'random','ucblcb'] # 'ucw_qp_min', 'ucw_ucb',

    rewards  = {}
    runtimes = {}
    colors   = {'optimal': 'purple', 'ucw_value': 'b', 'ucw_qp': 'c', 'ucw_qp_min': 'goldenrod', 'ucw_ucb': 'darkorange',
                'ucw_extreme': 'r', 'wiql': 'limegreen', 'random': 'brown','ucblcb':'k'}
    if 'ucblcb' in use_algos:
        print('-------------------------------------------------')
        print('UCBLCB')
        print('-------------------------------------------------')
        start                  = time.time()
        rewards['ucblcb']      = UcbLcb(simulator, n_episodes, n_epochs,0.2)
        runtimes['ucblcb']     = time.time() - start
